...
# Only pull day 0 and 14
X_humandat <- X_humandat[grepl("^0$|^14$", X_humandat$Time), ]
...
ggplot(pca_df_human, aes(x = PC1, y = PC2, fill = Time)) +
 ...
 labs(x = paste0("PC1 (", round(var_exp_human[1], 1), "%)"), 
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        aspect.ratio = 1)
#ggsave("humanimmob_PCA_14v0.svg", plot = humanimmob_PCA, width = 6, height = 4, units = "in")
```
...
Mouse_Human_TCR_reg <- Mouse_Human_TCR %>%
  mutate(Time = case_when(
    Time == "14" ~ 1,  # Replace "SA" with 1
    Time == "0" ~ 0,  # Replace "C" with 0
  ))
```

```{r}
# Model with all of the principal components in a regression model
model_list <- glm(Time ~ PC3, data = Mouse_Human_TCR_reg, family = binomial, maxit=100)
summary(model_list)
```
...
#ggsave("PC4PC3_plot_14v0.svg", plot = PC4PC3_plot, width = 6, height = 4, units = "in")
```
...
#ggsave("PC6PC4_plot_14v0.svg", plot = PC6PC4_plot, width = 6, height = 4, units = "in")
```
...
#ggsave("PC3PC6_plot_14v0.svg", plot = PC3PC6_plot, width = 6, height = 4, units = "in")
```

```{r}
#write.csv(Mouse_Human_TCR_reg, "Mouse_Human_TCR_reg14days.csv", row.names = FALSE)
```
*** END OF SCRIPT ***
